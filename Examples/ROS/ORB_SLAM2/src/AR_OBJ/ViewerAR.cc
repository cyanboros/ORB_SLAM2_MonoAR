...
//开启gl深度测试，使得pangolin只会绘制朝向镜头的那一面像素点，避免容易混淆的透视关系出现
    ...
    pangolin::Var<bool> menu_detectplane("menu.Insert OBJ",false,false);
    pangolin::Var<bool> menu_clear("menu.Clear All",false,false);
    pangolin::Var<bool> menu_drawim("menu.Draw Image",true,true);
    pangolin::Var<bool> menu_drawcube("menu.Draw OBJ",true,true);
    // pangolin::Var<float> menu_cubesize("menu. Cube Size",0.05,0.01,0.3);
    pangolin::Var<float> x_rotate("menu.X Rotate",0,0.1,360);
    pangolin::Var<float> y_rotate("menu.Y Rotate",0,0.1,360);
    pangolin::Var<float> z_rotate("menu.Z Rotate",0,0.1,360);
    pangolin::Var<float> menu_scale("menu.OBJ Scale",1,0.1,3);
